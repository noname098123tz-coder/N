...
local function SafeCall(fn, budget, ...)
    ...
    if not ok then
        Runtime.Errors += 1
        warn(("[BIB v7.1] ERR#%d → %s"):format(Runtime.Errors, tostring(err)))
        if Runtime.Errors >= Runtime.MaxErrors then
            warn("[BIB v7.1] error cap — killed")
            Runtime.Dead = true
        end
    end
    ...
    if budget and elapsed > budget then
        ...
        -- warn(("[BIB v7.1] budget overrun: %.2fms / %.2fms"):format(elapsed*1000, budget*1000))
    end
end
...
function WM.Tick()
    ...
    if (now - WM.LastUpdate) >= 0.5 then  -- update text 2x/sec max
        WM.LastUpdate = now
        SafeSet(WM.D,"Text",("BIBILABU v7.1  |  errs:%d  |  %ds"):format(
...
function Menu.Build()
    ...
    MD("Text",{Text="BIBILABU HUB  v7.1",Position=Vector2.new(p.X+12,p.Y+8),Size=15,Color=TPRI,Outline=true,OutlineColor=Color3.fromRGB(0,0,0)})
    ...
    for idx,row in ipairs(rows) do
        ...
        elseif cfg.Type=="slider" then
            ...
            local mn=cfg.Min or 0; local mx=cfg.Max or 100; local ratio=(cfg.Value-mn)/(mx-mn)
    ...
    if mb and (now-Menu.ClickCD)>0.15 and not Menu.Drag then
        for _,row in ipairs(Menu.Rows) do
            if InB(row.B,mp) then
                Menu.ClickCD=now
                if row.Type=="tab" then Menu.ActiveTab=row.Tab; Menu.Build()
...
Track(UserInputService.InputBegan:Connect(function(input,gp)
    ...
    if input.KeyCode==KeyMap[S("MenuKey")  or "Insert"] then
        ...
        if Menu.Vis then Menu.Build() else Menu.ClearDraw() end
    end
end))
...
SafeCall(function()
    ...
    Menu.Build()
    Runtime.Loaded=true
    Notif.Push("BIBILABU HUB","v7.1 live — frame-split ESP, zero freeze, that's what the hell is going on boss man.",4,AC)
end)

print("[BIBILABU HUB v7.1] budgeted. split. live.")
